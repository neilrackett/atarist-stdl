 ...
 */
/* Test the software mouse cursor
 *
 * SDL 1.2 test program (public domain), ported to STDL.
 * Port notes:
 *  - 320x200x4 video mode; the background fill colour is a palette
 *    index (the original's 0x664422 truncates to index 2, which is
 *    kept for the same visual effect)
 *  - the cursor is STDL's save-under software cursor: black pixels
 *    render as colour 0, white as colour 15, "inverted" as black
 *  - mouse button cycles through the three cursors, ESC quits
 */
...
static SDL_Cursor *create_arrow_cursor(void)
{
  int i, row, col;
  Uint8 data[4*32];
  Uint8 mask[4*32];
...
int main(int argc, char *argv[])
{
	...
	screen = SDL_SetVideoMode(320,200,4,SDL_ANYFORMAT);
	if (screen==NULL) {
