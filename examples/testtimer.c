...
/* Test program to check the resolution of the SDL timer on the current
   ...
   Port notes: unchanged apart from this header and printf formats.
   STDL timers are cooperative - callbacks fire inside SDL_Delay -
   and the clock underneath is the ST's 200Hz tick, so the best
   possible resolution is 5ms.
*/
...
{
	printf("Timer %lu : param = %d\n",
		(unsigned long)interval, (int)(uintptr_t)param);
	return interval;
}
